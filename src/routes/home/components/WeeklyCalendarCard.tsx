...
export function WeeklyCalendarCard({ weeklySummary }: WeeklyCalendarCardProps) {
  const firstActiveDateKey = weeklySummary.calendarDays.find((day) => day.sessions.length > 0)?.dateKey;
  const [selectedDateKey, setSelectedDateKey] = useState(firstActiveDateKey ?? weeklySummary.calendarDays[6]?.dateKey ?? '');

  const selectedDay = useMemo(
    () => weeklySummary.calendarDays.find((day) => day.dateKey === selectedDateKey) ?? weeklySummary.calendarDays[6],
  ...
  return (
    ...
    >
      ...
      <Text as="p" typography="t7" css={{ color: colors.grey600, marginTop: 6 }}>
        ...
        {weeklySummary.calendarDays.map((day) => {
          ...
          return (
            ...
            >
              ...
              <Text as="span" typography="t6" fontWeight="bold" css={{ color: tone.text }}>
                {day.dayNumber}
              </Text>
              <div
                css={{
                  ...
                  height: 10,
                  borderRadius: '999px',
                  background: day.sessions.length > 0 ? tone.text : 'rgba(120, 120, 120, 0.18)',
                }}
              />
            </button>
          );
      ...
      >
        ...
        {selectedSession ? (
          <div css={{ display: 'flex', flexDirection: 'column', gap: 6, marginTop: 10 }}>
            <Text as="div" typography="t7" css={{ color: colors.grey700 }}>
              대상: {formatSessionLabel(selectedSession)}
            </Text>
            <Text as="div" typography="t7" css={{ color: colors.grey700 }}>
              분노 게이지: {selectedSession.angerBefore} → {selectedSession.angerAfter}
            </Text>
            <Text as="div" typography="t7" css={{ color: colors.grey700 }}>
          ...
          <Text as="p" typography="t7" css={{ color: colors.grey500, marginTop: 10 }}>
            이 날은 기록이 없어요.
          </Text>
        )}
      </div>
